...
func runReporter(r *report) {
	...
	// 重要！！ 这里循环读取r.result会阻塞， 每次有一个数据进来就会循环一次。
	// 直到通道被关闭(在requester.finish()里面)，退出循环
...
func (r *report) finalize(total time.Duration) {
	r.total = total  // summary : total 这个total是在外面算的总时间
	r.rps = float64(r.numRes) / r.total.Seconds() // summary : qps = n/total
	r.average = r.avgTotal / float64(len(r.lats))  // summary : average

	//4,4 这块怎么算后续再看看
	r.avgConn = r.avgConn / float64(len(r.lats))  // detail 5个中的average
...
//latency = 延迟，延时。 例如，一个指令发出 到 抵达 耗用的时间
